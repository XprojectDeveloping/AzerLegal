...
const Lang = ({ toggle, switchLang, langs, scrolledFromTop }) => {
  ...
  useEffect(() => {
    ...
    const savedLang = localStorage.getItem("azerlegal") || "az";
    const validLang = langs.includes(pathLang) ? pathLang : savedLang;

    if (validLang !== language) {
      setLanguage(validLang);
      localStorage.setItem("azerlegal", validLang);
    }
  ...
  const langSwitcher = async (targetLang) => {
    ...
    localStorage.setItem("azerlegal", targetLang);
  ...
  return (
    <div className="relative bg-[#EAEDED] py-[0.7rem] px-[2rem] rounded-[0.4rem] text-[#000000]">
      <div
        onClick={toggle}
      ...
      {switchLang && (
        <div className="absolute mt-4 top-[3.2rem] w-[84px] left-[1px] flex flex-col gap-[1rem] text-left overflow-hidden items-center justify-center bg-[#fff] p-[5] rounded-[0.4rem] first">
          {selectedLangs?.map((lang, index) => (
            ...
            >
              {lang}
              <img src="/img/header/header-lang-arrow.svg" alt="" />
            </button>
          ))}
        </div>
      )}
